...
def load_dataset(in_file, batch_size):
    ...
    def _extract_fn(data_record):
        in_features = {
            ...
            #'x': tf.io.FixedLenFeature([n_points], tf.float32),
            #'y': tf.io.FixedLenFeature([n_points], tf.float32),
            #'z': tf.io.FixedLenFeature([n_points], tf.float32),
            #'A': tf.io.FixedLenFeature([n_points], tf.float32),
